...
void pd_url_parse_link(struct pd_univec * vec, GumboNode * node) {
  ...
  if (node->v.element.tag == GUMBO_TAG_A &&
    ...
    if (_pd_url_is_imgref(href->value)) {
      ...
      u->link = strdup(href->value);
      u->added = 0;
      u->date = _pd_url_date_from_addr(u->link);
      u->hash = 0;
...
void pd_url_wmessage(struct pbc_env * env, struct pbc_slice * slice, struct pd_univec * vec) {
  ...
  for (i = 0; i < url_n; ++i) {
    ...
    pbc_wmessage_string (url, "link",  ((struct pd_url *)(PD_UNIVEC_AT(vec,i)))->link,  -1);
    pbc_wmessage_integer(url, "added", ((struct pd_url *)(PD_UNIVEC_AT(vec,i)))->added, -1);
    pbc_wmessage_integer(url, "date",  ((struct pd_url *)(PD_UNIVEC_AT(vec,i)))->date,  -1);
    pbc_wmessage_integer(url, "hash",  ((struct pd_url *)(PD_UNIVEC_AT(vec,i)))->hash,  -1);
  }
